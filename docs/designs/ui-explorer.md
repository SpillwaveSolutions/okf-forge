...
| 10 | Catalog groups are ordered so `(root)` sits with its siblings, not first | agent |
| 11 | The validation badge is legible against the elevated surface | agent |
| 12 | Graph node labels do not overlap illegibly at the default hop count | agent |
